 ...
 * @param {number} [opts.port]           — bind port; defaults to MCP_PORT env
 * @param {string} [opts.host]           — bind host; defaults to MCP_BIND_HOST env
 * @returns {Promise<{ url: string }>}
 */
async function startMcpServer({ config, db, slackApp, port, host } = {}) {
    ...
    const { Server, StreamableHTTPServerTransport } = loadMcpSdk();
    ...
    app.all('/mcp/:sessionId', async (req, res) => {
        ...
        if (!entry) {
            ...
            // Wire our single tool. The handler is closed over
            // `connectionContext` so the per-call code knows which session
            // and which Slack channel/thread to post to.
            const connectionContext = { sessionId, db, slackApp, config };
            registerAskUserTool(mcpServer, connectionContext);
    ...
    const url = `http://${bindHost}:${bindPort}/mcp`;
 ...
 */
function registerAskUserTool(mcpServer, ctx) {
    ...
    // tools/call
    mcpServer.setRequestHandler({ method: 'tools/call' }, async (req) => {
