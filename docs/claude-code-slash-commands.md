...
per-class post-processing. Applied only to sessions whose `cli_type` is
`claude`; Codex/Gemini command sets are different and keep the old
passthrough.
...
### Pass-through (turn-starting — Stop hook posts the reply)

`/init`, `/review`, `/security-review`, `/pr-comments`, `/btw`, and every
custom skill / plugin command (`/pay-ops-production`, …). Unknown commands
print `Unknown command: /x` locally — the CLI starts no turn, so the thread
stays silent until the inflight watchdog reports; acceptable, since the
command echo message ("Sent `/x` to the session") is still posted.

## Where the code lives

- `src/channels/slack/socket.js` — `CLAUDE_PANEL_COMMANDS` /
  `CLAUDE_PRINT_COMMANDS` / `CLAUDE_BLOCKED_COMMANDS` (module top), intercept
  in `_processCommand`, `_handleModelCommand`, `_handleClaudeLocalCommand`,
  `_injectLocalCommand`, `_scrapeLocalCommandResult`.
- `src/cli/claude-adapter.js` — `supportsModelSwitch: true`,
  `modelSwitchConfirmRegex`.
- `src/cli/codex-adapter.js`, `src/cli/gemini-adapter.js` —
